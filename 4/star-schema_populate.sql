...
INSERT INTO f_anomalia(id_utilizador, id_tempo, id_local, id_lingua, tipo_anomalia, com_proposta) (
    ...
    SELECT id_utilizador, id_tempo, id_local, id_lingua, 'anomalia traducao', incidencia.anomalia_id IN (SELECT anomalia_id FROM correcao)
	...
	UNION
	SELECT id_utilizador, id_tempo, id_local, id_lingua, 'anomalia redacao', incidencia.anomalia_id IN (SELECT anomalia_id FROM correcao)
		FROM anomalia
		INNER JOIN incidencia ON incidencia.anomalia_id = anomalia.id
		INNER JOIN item ON item.id = incidencia.item_id
		INNER JOIN d_utilizador ON incidencia.email = d_utilizador.email
